...
class catalog
{
    ...
    var     $class_constants    = array(
        ...
        'photo_width'        => array(
            'type'  => 'int',
            'value' => 240,
            'label' => '<span style="color:red;">Ширина</span> большой картинки (px)',
        ),
        'thum_height'        => array('type' => 'int', 'value' => 120, 'label' => 'Высота иконки (px)'),
        'thum_width'         => array(
            'type'  => 'int',
            'value' => 150,
            'label' => '<span style="color:red;">Ширина</span> иконки (px)',
        ),
        'catalog_portion'    => array('type' => 'int', 'value' => 3, 'label' => 'Кол-во позиций на странице'),
        'num_news_prod_main' => array(
            'type'  => 'int',
            'value' => 4,
            'label' => 'Количество новинок<br />на главной странице',
        ),
        'num_news_prod'      => array('type' => 'int', 'value' => 3, 'label' => 'Количество новинок<br />в анонсе'),
    );

    var     $sub_menu           = array(//'price_list'=>array('name'=>'Загрузка прайс-листа', 'script_file'=>'price_list.php'),
    );
    ...
    public function set_factor($factor, $currency)
    {
        if (is_file($this->cur_file)) {
            if ($fp = @fopen($this->cur_file, 'w')) {
                @fwrite($fp, $this->cur_record($factor, $currency));
                fclose($fp);
            }
        }
    }
    ...
    public function cur_record($str = '', $currency = '')
    {
        $currency = preg_replace('/[^a-z]/i', '', $currency);
        $record   = '<?php defined(\'SYSPATH\') OR die(\'No direct access allowed\');
';
...
class gallery
{
    ...
    public function set_factor($factor = 0, $currency = 'USD')
    {
        $factor   = str_replace(',', '.', $factor);
        $currency = strtoupper(substr(trim(strip_tags($currency)), 0, 3));
        $this->catalog_object->set_factor($factor, $currency);
    }
    ...
    public function showcase_items()
    {
        ...
        $result = false;
        if ($res = $db->select_obj($this->photo_table, 'id,cat_id,item_name,photo,short,price,discount,quantity', 'publish=1 AND showcase=1 ORDER BY cat_id,sort_order')) {
    ...
    function list_cat_items($cat_id, $if_basket, $from = 0)
    {
        ...
        $res_item = $db->select_obj($this->photo_table, 'id,cat_id,item_name,photo,c_links,short,price,discount,quantity', 'publish=1 and cat_id=\'' . $cat_id . '\' order by sort_order' . $r_txt, 'file: ' . __FILE__ . 'line:' . __LINE__);
        ...
        if ($res_item) {
            foreach ($res_item as $at) {
                ...
                $at->discount = number_format(floatval($at->discount), 1, '.', ' ');
                if ($this->cur_factor > 0) {
                    $at->ruprice    = number_format(floatval($at->price * $this->cur_factor), 1, '.', ' ');
                    $at->rudiscount = number_format(floatval($at->discount * $this->cur_factor), 1, '.', ' ');
                }
                if ($if_basket) {
    ...
    function count_basket_items($data = array())
    {
        ...
        if ( ! empty($data)) {
            ...
            $fields = 'id,cat_id,item_name,photo,short,price,discount,quantity';
            $id_arr = array();
            $prices = array();
            ...
            foreach ($data as $key => $val) {
                ...
                $prices[] = 'discount*' . (int) $val . ' as sumdiscount_' . (int) $key;
            }
